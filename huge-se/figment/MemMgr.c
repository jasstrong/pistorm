...
/* GetSP provided by figment_shim.h */
#if 0  /* MPW inline asm — replaced by GCC version in shim */
	static pascal  ulong   GetSP(void) = {0x2E8F};		/*	Move.l	A7,(Sp)		*/
#elif !defined(powerc) && !defined(StarTrek)
	/* GetSP from shim */
#else
...
void  c_InitZone(GrowZoneProcPtr pGrowZone, long numMasters, void* limitPtr, void* startPtr)
	{
	stdHeap* newZone;

	FIG_DBG("InitZone start=", (unsigned long)startPtr);
	FIG_DBG("  limit=", (unsigned long)limitPtr);
	FIG_DBG("  moreMast=", numMasters);

	/* Figment zone header is 120 bytes vs old MM's 52.  patch-rom.py pads
	 * the ROM resource map by 68 bytes and enlarges the allocation (MOVEQ
	 * at $DA13) so the map data lands after the Figment header.
	 *
	 * The RM computes the map base from zone+$2E (old MM's allocPtr field).
	 * After zone creation, write Figment's heapStart there so the RM finds
	 * the map at the right offset.  This overlaps Figment's
	 * lowestRemovableBlock field which is harmless for this tiny zone. */
	{
		unsigned long zoneSize = (unsigned long)limitPtr - (unsigned long)startPtr;
		if (zoneSize < 4096 && (unsigned long)startPtr >= 0x2000) {
			/* Flag: after CreateNewHeap, fixup zone+$2E */
			/* We'll do it after the LMSetTheZone below */
		}
	}
...
OSErr c_NewHandle(Handle* theHandle, unsigned long trapWord, Size requestedSize)
	{
	stdHeap*			curHeap;
	handleBlock*	newBlock;
	Handle			newMasterPtr;

	FIG_DBG("NewHandle size=", requestedSize);
	FIG_DBG("  trapWord=", trapWord);
	...
	if (curHeap->firstFreeMP == 0)
		...
		if (noErr != (theErr = AllocateMoreMasters(curHeap)))
			{
			FIG_DBG("  MoreMasters FAIL=", theErr);
			FlushCacheIfNeeded(curHeap);
	...
	FIG_DBG("  reqBlockSize=", reqBlockSize);
	FIG_DBG("  firstFree=", (unsigned long)curHeap->firstFree);
	FIG_DBG("  favoredFree=", (unsigned long)curHeap->favoredFree);
	if (curHeap->firstFree)
		FIG_DBG("  firstFree->size=", curHeap->firstFree->size);
	FIG_DBG("  totalFree=", curHeap->totalFree);

	/* allocate new block — try fast path, then slow path */
	newBlock = (handleBlock *) NewBlockFast(reqBlockSize, curHeap);
	if (nil == newBlock)
		{
		FIG_DBG("  Fast FAIL, trying slow", reqBlockSize);
		newBlock = (handleBlock *) NewBlock(reqBlockSize, kAllocateLow, curHeap);
		}
	if (nil == newBlock)
		{
		FIG_DBG("  NewBlock FAIL", 0);
		FlushCacheIfNeeded(curHeap);
		DbgCheckHeap(curHeap);
		*theHandle = nil;
		return LMSetMemErr(memFullErr);
...
OSErr c_NewEmptyHandle(Handle* theHandle, unsigned long trapWord)
	{
	stdHeap*		curHeap;
	Handle		newMasterPtr;
...
OSErr c_NewHandleHi(Handle* theHandle, unsigned long trapWord, Size requestedSize)
	{
	stdHeap*			curHeap;
	handleBlock*	newBlock;
	Handle			newMasterPtr;
 ...
 */
Handle c_RecoverHandle(unsigned long trapWord, const Ptr textPtr)
{
...
OSErr c_NewPtr(Ptr* thePtr, unsigned long trapWord, Size requestedSize)
	{
	stdHeap*		curHeap;
	ptrBlock*	newBlock;
	blockSize_t	reqBlockSize;

	FIG_DBG("NewPtr size=", requestedSize);
	FIG_DBG("  totalFree=", (OperateOnSysHeap(trapWord) ? LMGetSysZone() : LMGetTheZone())->totalFree);
	...
	DbgValidateTrapWord(trapWord);
	if (!IsHeap(curHeap = OperateOnSysHeap(trapWord) ? LMGetSysZone() : LMGetTheZone())) {
		FIG_DBG("  BAD HEAP! curHeap=", (unsigned long)curHeap);
		return LMSetMemErr(memAZErr);
		}
	...
	if (nil == (newBlock = NewBlock(reqBlockSize, kAllocateLow, curHeap)))
		{
		/* allocation failed */
		FIG_DBG("  NewPtr FAIL in zone=", (unsigned long)curHeap);
		FIG_DBG("    reqBlockSize=", reqBlockSize);
		...
		*thePtr = (Ptr)newBlock->data;
		FIG_DBG("  NewPtr OK at=", (unsigned long)newBlock->data);
		FIG_DBG("  totalFree after=", curHeap->totalFree);
		return LMSetMemErr(noErr);
		}
	}
...
long c_FreeMem(unsigned long trapWord)
	{
	stdHeap*	curHeap;
...
Size c_MaxMem(unsigned long trapWord, Size *growSize)
	{
	stdHeap*		curHeap;
	freeBlock*	free;
...
Size c_CompactMem(unsigned long trapWord, Size sizeNeeded)
	{
	stdHeap*		curHeap;
	blockSize_t	reqBlockSize;
...
OSErr c_ReserveMem(unsigned long trapWord, Size sizeNeeded)
	{
	stdHeap*		curHeap;
	blockSize_t	reqBlockSize ;
	freeBlock*	free;
...
OSErr c_PurgeMem(unsigned long trapWord, Size sizeNeeded)
	{
	stdHeap*		curHeap;
	freeBlock*	free;
	OSErr			theErr;
...
Size c_MaxBlock(unsigned long trapWord)
	{
	stdHeap*		curHeap;
...
Size c_PurgeSpace(unsigned long trapWord, Size* contig)
	{
	stdHeap* curHeap;
	blockSize_t	total;
...
OSErr c_SetZoneCompatibilityFlags(THz theZone, unsigned long theFlags)
	{
...
/* this is a way for ProcessMgr inform the heap manager of special compatibility hacks */
unsigned long c_GetZoneCompatibilityFlags(THz theZone)
	{
